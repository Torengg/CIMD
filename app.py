...
def initialise_session() -> None:
    for key, value in {
        ...
        "decision_time_seconds": None, "decision_saved": False,
    }.items():
...
def save_response(record: dict) -> tuple[bool, str]:
    """Append one response to the connected Google Sheet."""
    if not GOOGLE_SHEETS_URL or not GOOGLE_SHEETS_KEY:
        return False, "The Google Sheet connection has not been configured yet."
    try:
        response = requests.post(GOOGLE_SHEETS_URL, json={**record, "api_key": GOOGLE_SHEETS_KEY}, timeout=15)
...
def rating(label: str, key: str, low: str, high: str) -> int:
    ...
    return answer
def progress(current: int) -> None:
    st.progress(current / 2, text=f"Step {current} of 2")
...
def render_offer() -> None:
    ...
    if chose_cart or chose_not_interested:
        ...
        if chose_cart or chose_not_interested:
            st.session_state.decision = "ADD_TO_CART" if chose_cart else "NOT_INTERESTED"
            st.session_state.decision_time_seconds = round(time.perf_counter() - st.session_state.ad_exposure_started_at, 3)
        ...
        if st.button("Retry saving my response", type="primary", use_container_width=True):
            saved, message = save_response(decision_record())
            if saved:
                st.session_state.decision_saved = True
                st.session_state.page = "thank_you"
...
def main() -> None:
    ...
    {"welcome": render_welcome, "offer": render_offer, "thank_you": render_thank_you}[st.session_state.page]()
